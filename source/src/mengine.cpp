...
{
	return load(mrender::getFileReader(), mrender::getAllocator(), _filePath, _size);
}

void unload(void* _ptr)
{
	bx::free(mrender::getAllocator(), _ptr);
}
...
{
	return loadShader(mrender::getFileReader(), _name);
}
...
{
	return loadProgram(mrender::getFileReader(), _vsName, _fsName);
}
...
{
	return loadTexture(mrender::getFileReader(), _name, _flags, _skip, _info, _orientation);
}
...
void Mesh::load(bx::ReaderSeekerI* _reader, bool _ramcopy)
{
	...
	bx::AllocatorI* allocator = mrender::getAllocator();
...
{
	MeshState* state = (MeshState*)bx::alloc(mrender::getAllocator(), sizeof(MeshState));
	return state;
}

void meshStateDestroy(MeshState* _meshState)
{
	bx::free(mrender::getAllocator(), _meshState);
}
